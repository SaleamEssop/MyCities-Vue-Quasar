<template>
  ...
  <template v-else>
    ...
    <div v-if="billDetails?.meters?.water_meters.length">
      <div style="border: 1px dotted;margin: 20px" v-for="waterMeter in (billDetails?.meters?.water_meters || [])" :key="waterMeter.meter_details.id">
          ...
          <q-list bordered separator>
            <q-item class="item-style">
              ...
              <q-item-section class="text-blue-9 medium-text">{{
                  unitFormat.unitFormat(waterMeter.usage.predictive_monthly_usage, waterMeter?.meter_details?.meter_type_id)
                }}
              </q-item-section>
            ...
            <q-item class="item-style"
                    v-for="(waterInAdditionalCost,i2) in waterMeter?.data?.water_out?.predictive?.additional_costs || []"
                    :key="i2">
              <q-item-section class="item-styling">{{ waterInAdditionalCost.title }}</q-item-section>
              <q-item-section side class="item-styling">R {{ numberFormat.numberFormat(waterInAdditionalCost?.cost || 0) }}</q-item-section>
            </q-item>
    ...
    <div v-if="billDetails?.meters?.electricity_meters.length">
      <div style="border: 1px dotted;margin: 20px" v-for="electricityMeter in (billDetails?.meters?.electricity_meters || [])"
        ...
        <q-card flat class="q-ma-sm q-pa-sm">
          <q-list bordered separator>
            <q-item class="item-style">
              ...
              <q-item-section class="text-blue-9 medium-text">{{
                  unitFormat.unitFormat(electricityMeter.usage.predictive_monthly_usage, electricityMeter?.meter_details?.meter_type_id)
                }}
              </q-item-section>
              <q-item-section side class="item-styling">R {{
                  numberFormat.numberFormat(electricityMeter?.data?.predictive || 0)
                }}
              </q-item-section>
            </q-item>
            <q-item class="item-style"
                    v-for="(electricityAdditionalCost,i2) in electricityMeter?.data?.electricity?.predictive?.additional_costs || []"
                    :key="i2">
    ...
    <q-card flat class="bg-grey-4 q-ma-sm q-pa-sm">
      <div style="flex-direction: column" class="flex column items-center" aria-label="heading">
        ...
        <span class="large-text">R {{ numberFormat.numberFormat(billDetails?.total_including_vat || 0) }}</span>
      </div>
    </q-card>
    ...
    <q-card flat class="bg-white q-ma-sm q-pa-sm" style="padding-bottom: 40px">
      <q-banner dense inline-actions class="text-white bg-red">
        Note: The amount is calculated based on your inputs and may differ slightly from your actual municipal bill.
      </q-banner>
    </q-card>
  </template>

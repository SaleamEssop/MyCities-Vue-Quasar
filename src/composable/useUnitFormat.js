...
export const useUnitFormat = () => {
  ...
  return {
    ...
    unitFormat(value, type, fractions = false) {
      ...
      if (type === WATER_METER) {
        if (fractions){
          return `${formatNumberWithLeading4Zeroes(formatterFraction.format(value))} kL`;
        }else{
        ...
        if (fractions){
          console.log(value);
          return `${formatNumberWithLeading5Zeroes(electricytFormatterFraction.format(value))} kWh`;
        }else{
